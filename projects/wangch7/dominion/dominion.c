...
// Refactoring - below 5 cards are implemented in their own functions, rather than inside of switch statement. 
//               Then, each card function will be called from case.
int adventurerRefactor(int drawntreasure, struct gameState *state, int currentPlayer)
{
  int cardDrawn;
  int z = 0;
...
int mineRefactor(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2)
{
  int i = 0;
  int j;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      adventurerRefactor(drawntreasure, state, currentPlayer);
      return 0;
 //      while(drawntreasure<2){
 ...
 //      return 0;
    ...
    case council_room:
      //+4 Cards
      countcil_roomRefactor(currentPlayer, state, handPos);
 //      for (i = 0; i < 4; i++)
	// {
	//   drawCard(currentPlayer, state);
	// }
 ...
 //      discardCard(handPos, currentPlayer, state, 0);
    ...
    case mine:
      mineRefactor(currentPlayer, state, handPos, choice1, choice2);
 //      j = state->hand[currentPlayer][choice1];  //store card we will trash
 ...
 //      for (i = 0; i < state->handCount[currentPlayer]; i++)
    ...
    case remodel:
      remodelRefactor(currentPlayer, state, handPos, choice1, choice2);
 //      j = state->hand[currentPlayer][choice1];  //store card we will trash
